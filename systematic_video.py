...
def get_stress(stress):
    ...
    avg = [0] * iterations
    minn = [9999999999999999] * iterations
    maxx = [0] * iterations
    for point in stress:
        avg[point[0]] += point[1]
...
for it in range(3,4):
    print('{}:'.format(it))
    range1 = get_range("*_maj.txt", it)
    range2 = get_range("*_wcr.txt", min(it,19))
    ...
    plt.title(str(it+1) + '/15 iterations', fontweight='bold')
    ...
    plt.savefig('video/15_' + str(it) + ".png", bbox_inches='tight')
